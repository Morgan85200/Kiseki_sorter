...
function display() {
  ...
  const charNameDisp = name => {
    const charName = reduceTextWidth(name, 'Arial 12.8px', 250);
...
function result(imageNum = 10) {
  ...
  const res = (char, num) => {
    const charName = reduceTextWidth(char.name, 'Arial 12px', 250);
    const charTooltip = char.name !== charName ? char.name : '';
    return `<div class="result"><div class="left">${num}</div><div class="right"><span class="span_text" title ="${charTooltip}">${charName}</span></div></div>`;
  }
